...
def readAndRun(input_file, header_file, output_directory, k_bottom, k_top, runs, algorithm):
    ...
    for rank in range(k_bottom, k_top):
        if algorithm == 'S':
            ...
            targets[rank] = target
            np.savetxt("../Data/BreastCancerOriginal/target" + str(rank) + ".csv",target,delimiter=",",fmt="%d")
...
def main():
    ...
    readAndRun(data_directory + input_file, data_directory + header_file, results_directory, 4, 5, 100, 'S')
